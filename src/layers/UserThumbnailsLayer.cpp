...
void UserThumbnailsLayer::loadUserThumbnails() {
    // Call API to get user's uploaded thumbnails
    ThumbnailAPI::get().getUserUploads(m_username, [this](bool success, std::string const& response) {
        if (!this->getParent()) return;
        
        m_loadingLabel->setVisible(false);
        
        if (!success) {
            showError("Failed to load thumbnails");
            return;
        }
        ...
        // Check if response has levelIds array
        ...
        // Alternative: check for levels array with objects
        ...
        if (levelIds.empty()) {
            showError("No thumbnails found");
            return;
        }
        
        displayLevels(levelIds);
    });
}
...
void UserThumbnailsLayer::displayLevels(std::vector<int> const& levelIds) {
    ...
    // Set content height for scroll layer
...
void UserThumbnailsLayer::onLevelClicked(CCObject* sender) {
    ...
    // Try to load and show the level
    auto glm = GameLevelManager::sharedState();
    if (!glm) return;
    
    // Search for the level online
