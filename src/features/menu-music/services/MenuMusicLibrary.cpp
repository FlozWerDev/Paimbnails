...
void MenuMusicLibrary::removeTrack(const std::string& id, bool deleteFiles) {
    // Tomar una copia antes de borrar del vector para poder limpiar archivos
    MusicTrack copy;
    bool found = false;
    for (const auto& t : m_tracks) {
        if (t.id == id) { copy = t; found = true; break; }
    }
    if (!found) return;
    ...
    // Sacar tambien de todas las playlists
    ...
    // Limpieza de archivos (solo si fue descargado)
