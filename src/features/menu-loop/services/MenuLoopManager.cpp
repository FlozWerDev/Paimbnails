...
static bool isSupportedFile(const std::string_view path) {
    ...
    // Formatos que FMOD (la engine de GD) puede reproducir directamente.
    //
    // IMPORTANTE: FMOD Core en GD NO soporta AAC (m4a) fuera de iOS por
    // motivos de licencia, y NO soporta Opus/WebM en el path habitual.
    // Aunque el fichero se descargue, queda silencioso al cargarlo. Por
    // eso no los incluimos aqui. Si el usuario baja de YouTube, el
    // YtDlpDownloader usa ffmpeg para recodificar a mp3 antes de
    // devolver el path, asi que siempre llegamos con un .mp3 valido.
