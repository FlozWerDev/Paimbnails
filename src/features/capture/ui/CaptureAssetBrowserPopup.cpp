...
void CaptureAssetBrowserPopup::scanViewportObjects() {
    ...
    const bool needRecordOriginals = s_originalAssetVisibilities.empty();

    if (!pl->m_objects) return;

    // O(1) lookup; avoids O(objects²) linear scan of s_originalAssetVisibilities.
    std::unordered_map<cocos2d::CCNode*, bool> originalLookup;
    if (needRecordOriginals) {
        // Reserve up front; one record per object.
        s_originalAssetVisibilities.reserve(s_originalAssetVisibilities.size() +
            static_cast<size_t>(pl->m_objects->count()));
    } else {
        originalLookup.reserve(s_originalAssetVisibilities.size());
        for (auto const& rec : s_originalAssetVisibilities) {
            originalLookup.emplace(rec.node, rec.visible);
        }
    }
    ...
    for (auto* obj : CCArrayExt<GameObject*>(pl->m_objects)) {
        ...
        if (it == idToGroupIdx.end()) {
            ...
            // On re-open, prefer original visibility over possibly-edited current state.
            if (!needRecordOriginals) {
                if (auto found = originalLookup.find(obj); found != originalLookup.end()) {
                    group.originalVisible = found->second;
                }
            }
        ...
        if (needRecordOriginals) {
            // Each object visited once; append directly (avoids O(objects²) hot path).
            s_originalAssetVisibilities.push_back({obj, obj->isVisible()});
        }
    }
    ...
    std::sort(m_categories.begin(), m_categories.end(), [&](const CategoryHeader& a, const CategoryHeader& b) {
        // Determine order from categoryKey
    ...
    for (int catIdx = 0; catIdx < static_cast<int>(m_categories.size()); ++catIdx) {
        ...
        // Category header row
        {
            ...
            if (onSpr && offSpr) {
                ...
                // tag offset to distinguish from group toggles
...
void CaptureAssetBrowserPopup::onToggleCategory(CCObject* sender) {
    ...
    // Defer: destroying the toggler while the dispatcher unwinds corrupts state.
...
void CaptureAssetBrowserPopup::onDoneBtn(CCObject* sender) {
    // Trigger recapture on parent preview
...
void CaptureAssetBrowserPopup::setAllObjectsVisible(bool visible) {
    auto* pl = PlayLayer::get();
    if (!pl || !pl->m_objects) return;

    // O(objects) single pass — equivalent to per-group but avoids O(groups × objects).
    for (auto* obj : CCArrayExt<GameObject*>(pl->m_objects)) {
...
void CaptureAssetBrowserPopup::onShowAllBtn(CCObject* sender) {
    for (auto& group : m_groups) group.visible = true;
...
void CaptureAssetBrowserPopup::onHideAllBtn(CCObject* sender) {
    for (auto& group : m_groups) group.visible = false;
