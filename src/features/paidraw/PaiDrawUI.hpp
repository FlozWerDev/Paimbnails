...
public:
    ...
    // Undo / redo locales (no se sincroniza por red en modo test)
    void undoLast();
    void redoLast();
...
public:
    ...
    void refreshState();
    // Per-frame tick (scheduled at 0.1s) that interpolates the round
    // countdown locally so the timer label updates smoothly between
    // server snapshots instead of jumping every poll.
