...
ThumbnailLoader::ThumbnailLoader() {
    ...
    // Menos tareas simultaneas en arranque/scroll: reduce picos de decode + uploads.
...
void ThumbnailLoader::drainPendingCallbacks() {
    ...
    // Purgar texturas no usadas throttled (cada ~2s). Si la cola de callbacks
    // sigue grande, posponer purge en main para priorizar thumbs visibles.
...
void ThumbnailLoader::prefetchLevels(std::vector<int> const& levelIDs, int priority) {
    ...
    // limitar prefetch segun concurrencia configurada (setting thumbnail-concurrent-downloads)
...
void ThumbnailLoader::flushBatchDownloads() {
    ...
    // El batch API solo devuelve thumbnails estaticos (PNG). Las tareas GIF
    // (levelID negativo) deben ir por descarga individual para no mezclar bytes.
