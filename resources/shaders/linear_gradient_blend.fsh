...
void main() {
    ...
    if (stopAt <= 1) {
        gl_FragColor = texColor * pointColor(0) * v_fragmentColor;
        return;
    }
    ...
    for (int i = 0; i < 23; ++i) {
        ...
        if (t <= b) {
            ...
            gl_FragColor = texColor * mix(pointColor(i), pointColor(i + 1), localT) * v_fragmentColor;
            return;
        }
    }

    gl_FragColor = texColor * pointColor(stopAt - 1) * v_fragmentColor;
}
