...
#include "ImageLoadHelper.hpp"   // para PNG embebido (stb_image)
...
// Lectura little-endian segura (sin desbordar el buffer).
...
constexpr int kMaxDim = 1024; // los cursores nunca son grandes; corta abusos

// Decodifica el payload de una entrada de icono. `img`/`imgSize` apuntan al
// inicio del payload (BMP DIB con AND mask, o un PNG completo).
bool decodeIconImage(uint8_t const* img, size_t imgSize, DecodedFrame& out) {
    if (imgSize < 8) return false;

    // ── Caso PNG embebido (iconos Vista+) ──
    // Decodificamos directo con stb_image (sin crear CCTexture2D) para no
    // depender del contexto GL ni del main thread durante la importacion.
    ...
    // ── Caso BMP/DIB (BITMAPINFOHEADER) ──
    // El header tiene 40 bytes. El "height" del DIB es el DOBLE de la altura
    // real porque incluye la mascara AND (1bpp) apilada debajo del color.
    ...
    int32_t h = hRaw / 2; // altura real (color); la otra mitad es la AND mask
    if (h <= 0 || h > kMaxDim) return false;
    if (compress != 0) return false; // solo BI_RGB sin comprimir
    ...
    // El DIB se guarda bottom-up: la primera fila del buffer es la INFERIOR.
    // Escribimos top-down en out.rgba (fila 0 = arriba).
    ...
    if (bpp == 32) {
        // Filas alineadas a 4 bytes (32bpp ya lo esta). Orden BGRA.
    ...
    if (bpp == 24) {
        size_t rowBytes = ((static_cast<size_t>(w) * 3 + 3) / 4) * 4; // padding a 4
        // La AND mask viene despues del bloque de color.
    ...
    if (bpp == 8 || bpp == 4 || bpp == 1) {
        // Paleta indexada: BITMAPINFOHEADER seguido de la tabla de color.
        int paletteCount = 1 << bpp;
        uint8_t const* palette = img + headerSize;
        size_t paletteBytes = static_cast<size_t>(paletteCount) * 4; // BGRA(reservado)
        uint8_t const* bits = palette + paletteBytes;
        if (bits >= end) return false;

        size_t rowBits = static_cast<size_t>(w) * bpp;
        size_t rowBytes = ((rowBits + 31) / 32) * 4; // pad a 4
        size_t colorBytes = rowBytes * h;
...
// Decodifica un .ico/.cur a un solo frame (la imagen de mayor resolucion).
bool decodeIcoInternal(uint8_t const* data, size_t size, DecodedFrame& out) {
    ...
    // Elegir la entrada de mayor area.
    int bestIdx = -1;
    long bestArea = -1;
    ...
    if (imageOffset + bytesInRes > size || bytesInRes == 0) return false;
...
DecodeResult decodeAni(uint8_t const* data, size_t size) {
    ...
    // Estructura RIFF: "RIFF" <size> "ACON" { chunks }.
    // Chunks relevantes:
    //   "anih" : ANIHEADER (incluye jifRate por defecto y nFrames/nSteps)
    //   "rate" : array de tasas por step (jiffies = 1/60 s)
    //   "seq " : orden de reproduccion (indices a frames)
    //   "LIST" "fram" { "icon" <ico/cur> ... }  : los frames en si
    uint32_t defaultJiffies = 6;     // ~100ms (6/60s) por defecto
    std::vector<DecodedFrame> icons; // frames decodificados en orden de aparicion
    std::vector<uint32_t> rates;     // por step
    std::vector<uint32_t> seq;       // orden de reproduccion

    auto const* p = data + 12;       // tras "RIFF"<size>"ACON"
    ...
    while (p + 8 <= end) {
        ...
        if (memcmp(id, "anih", 4) == 0 && chunkSize >= 36) {
            ...
            // Recorrer sub-chunks "icon" dentro de la lista "fram".
            ...
            while (lp + 8 <= lend) {
                ...
                // chunks RIFF alineados a word
                lp = sBody + sSize + (sSize & 1);
            }
        }

        p = body + chunkSize + (chunkSize & 1); // alineacion a word
    }

    if (icons.empty()) { res.error = "ani_no_frames"; return res; }

    // Construir la secuencia final aplicando "seq" y "rate" si existen.
    ...
    for (size_t s = 0; s < steps; ++s) {
        ...
        DecodedFrame f = icons[iconIdx]; // copia (los frames pueden repetirse)
