// Comment Mentions — notifica cuando alguien te menciona en los comentarios
// de un nivel (daily, weekly, event o IDs personalizados).
//
...
// WebHelper y todo el estado de menciones vive exclusivamente en el main thread.
...
// Decodifica base64 URL-safe (los comentarios de GD usan '-' y '_'). Tolera
// tambien el alfabeto estandar '+' '/' y caracteres de padding/whitespace.
...
// Parsea una cadena "k<sep>v<sep>k<sep>v..." en un mapa.
...
// Lee un setting de string separado por comas en una lista, recortando espacios.
...
// POST a los servidores de GD. El callback corre en el main thread.
void gdRequest(std::string const& endpoint, std::string const& body,
    ...
    WebHelper::dispatch(std::move(req), "POST", std::string(GD_BASE) + endpoint,
        [cb = std::move(cb)](web::WebResponse res) {
            ...
            std::string b = res.string().unwrapOr("");
            while (!b.empty() && (b.back() == '\n' || b.back() == '\r' ||
                                  b.back() == ' ' || b.back() == '\t')) {
                b.pop_back();
            }
            if (b.empty() || b == "-1") { cb(false, ""); return; }
            cb(true, std::move(b));
        });
}
...
public:
    ...
    void startup() {
        ...
        // Primera ejecucion: usar el propio nombre de usuario como alias.
    ...
    // (Re)calcula los IDs especiales a vigilar. Main thread.
    void reloadLevels() {
...
private:
    ...
    // Main thread. Dispara una peticion de comentarios por cada nivel vigilado.
    void pollOnce() {
        if (!sBool("mentions-enabled")) return;
        updateAliases();
    ...
    void pollLevel(int levelID) {
        gdRequest("getGJComments21.php",
            ...
            [this](bool ok, std::string body) {
                ...
                // Si esta jugando y no se permite, no marcar como vistos para
                // re-detectarlos cuando vuelva al menu.
...
// Resuelve username -> accountID via getGJUsers20 y abre su ProfilePage.
// Reutiliza los helpers gdRequest/parseKV del namespace anonimo de este TU.
