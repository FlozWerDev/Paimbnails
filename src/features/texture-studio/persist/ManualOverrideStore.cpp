...
geode::Result<> ManualOverrideStore::save(std::filesystem::path const& path,
    ...
    if (!(hasC1 || hasC2 || hasGlow || hasOL)) {
        return Err("ManualOverrideStore::save: all masks empty for {}", path.string());
    }
    ...
    if (!parent.empty()) {
        ...
        if (ec) {
            return Err("create_directories {}: {}", parent.string(), ec.message());
        }
    }

    auto wr = file::writeBinary(path, buf);
    if (!wr) {
        return Err("writeBinary {}: {}", path.string(), wr.unwrapErr());
    }
    return Ok();
}
...
geode::Result<MaskSet> ManualOverrideStore::load(std::filesystem::path const& path) {
    ...
    if (!rd) {
        return Err("readBinary {}: {}", path.string(), rd.unwrapErr());
    }
...
geode::Result<> ManualOverrideStore::deleteForSlot(std::string_view slotId,
    ...
    if (ec) {
        return Err("remove {}: {}", path.string(), ec.message());
    }
    return Ok();
}
