...
namespace {
    ...
    static CCNode* makeDarkPanel(float w, float h, GLubyte alpha = 70) {
        // Reference style: dark interior + thin near-white border. Matches
        // the post cards on the Hub forum tab and the popup chrome so every
        // surface in the forum reads the same.
...
bool PostDetailPopup::init(Post const& post, CopyableFunction<void()> onChanged) {
    ...
    // Replace the vanilla brown popup frame with the dark rounded panel used
    // throughout the rest of the mod. Same chrome as the Hub forum tab so
...
void PostDetailPopup::rebuild() {
    ...
    // ── Layout (top → bottom) ───────────────────────────────────────────────
    //   header (icon + name + relative + absolute) ........ 34 px
    //   tags row (optional) ................................ 18 px
    //   description box (rounded panel + MD area) .......... 50 px
    //   action bar (Like / Report / Delete) ................. 28 px
    //   replies header label ................................ 16 px
    //   replies scroll ...................................... fills
    //   reply input + send .................................. 30 px
    ...
    float actionY = descBot - kRowGap - kActionH / 2.f;
    {
        ...
        // Use a CCMenu with a row layout so spacing stays uniform regardless
        // of the runtime label widths.
...
CCNode* PostDetailPopup::makeAuthorRow(Author const& author, int64_t when, float w) {
    ...
    // Slightly taller than before so the bigger avatar reads cleanly.
    float h = 34.f;
...
CCNode* PostDetailPopup::makeReplyCard(Reply const& r, float w) {
    ...
    //   row2 (mid): contenido (multi-line, wraps based on width)
    ...
    // Highlight the current user's own replies with a softer border so
    // moderators / authors can spot their own messages quickly.
    auto* acc = GJAccountManager::get();
    int myId = acc ? acc->m_accountID : 0;
