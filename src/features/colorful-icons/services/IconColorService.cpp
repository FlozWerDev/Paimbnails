...
float IconColorService::globalTime() {
    if (auto* d = CCDirector::sharedDirector()) {
        // CCDirector::getTotalFrames() / FPS would also work, but
        // getDeltaTime is sample-accurate. We use a per-call accumulator
        // backed by a static so the value monotonically increases.
        ...
        if (lastTotalFrames < 0.0f || frames < lastTotalFrames) {
            // First call or wrap-around: reset to a sane value.
...
IconColorTriple IconColorService::resolveRandom(IconDescriptor const& desc, PaimonIconConfig const& cfg) const {
    // Deterministic hash from (unlockType, iconID).
...
IconColorTriple IconColorService::resolveRainbow(IconDescriptor const& desc, PaimonIconConfig const& cfg, float t) const {
    // Each icon gets a phase offset proportional to its (unlockType, iconID)
    // so rows of icons produce a smooth sweep instead of all flashing at once.
    const float baseHue = std::fmod(t * cfg.rainbowSpeed * 60.0f, 360.0f);  // 60 deg/sec at speed 1
...
IconColorTriple IconColorService::resolveGradient(IconDescriptor const& desc, PaimonIconConfig const& cfg) const {
    ...
    IconColorTriple out;
    out.primary   = mixed;
    out.secondary = math::scaleSV(mixed, 1.0f, 0.7f);  // darker secondary
    out.glow      = math::scaleSV(mixed, 1.0f, 1.2f);  // brighter glow
    out.hasGlow   = true;
    return out;
}
...
IconColorTriple IconColorService::resolvePerGamemode(IconDescriptor const& desc, IconColorTriple const& base, PaimonIconConfig const& cfg) const {
    ...
    if (!slot) return base;  // fall back to player colors when slot is unset
    IconColorTriple out;
