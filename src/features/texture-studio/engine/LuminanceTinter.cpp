...
ImageBuffer LuminanceTinter::apply(ImageBuffer const& source,
    ...
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x < W; ++x) {
            ...
            // Outline guard: los pixeles muy oscuros son contorno. El pixel
            // original ya está en `dst` por la copia inicial, así que basta
            // con saltar el overlay para preservarlo intacto.
