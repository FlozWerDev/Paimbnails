...
VideoPlayer::~VideoPlayer() {
    ...
    if (!isOnMainThread()) {
        ...
        // PBO shutdown skipped: calling GL from non-GL thread is UB.
        // PBOs leak but are reclaimed when the GL context is destroyed.
        return;
    }
...
bool VideoPlayer::init(const std::string& videoPath, const VideoPlayerCreateOptions& options) {
    ...
    // RGBA staging buffer is allocated lazily in uploadFrame() — only the
    // CPU YUV→RGBA path needs it.
...
// Pre-allocate textures, PBOs and resolve FBO before the first frame to
// avoid GL allocation stalls on the main thread. Idempotent.
void VideoPlayer::prepareGPUPipeline() {
    ...
    if (!m_pboInitAttempted) {
        ...
        if (m_useGPUYuv) {
            ...
            // CPU YUV→RGBA path: allocate staging buffer early. uploadFrame()
            // defensively retries allocation if this fails.
            if (!m_rgbaBuffer) {
...
bool VideoPlayer::uploadFrame(const IVideoDecoder::Frame& frame) {
    ...
    // Lazy init fallback in case play() was skipped.
    if (!m_pboInitAttempted) {
        prepareGPUPipeline();
    }
    ...
    // Detect a laggy frame (busy UI); defer expensive YUV conversion so we don't make it worse.
    auto* director = cocos2d::CCDirector::get();
    if (!director) return false;
    float dt = director->getDeltaTime();
    bool isFrameLag = dt > 0.020f;  // > 20ms = severe lag @ 60fps

    // First frame during lag: defer to avoid blocking further. Later frames process normally.
    if (isFrameLag && !m_hasVisibleFrame) {
        return false;  // retry next frame when the UI is less busy
    }

    if (m_pboUploader.isInitialized()) {
        // Zero-copy path: convert YUV into mapped PBO memory. On the first
        // frame also fill the CPU staging buffer so copyCurrentFramePixels()
        // can cache it for the thumbnail disk cache.
    ...
    // No PBO available: convert into the staging buffer for direct upload.
    yuvToRgba(frame.planeY, frame.strideY,
              frame.planeCb, frame.strideCb,
              frame.planeCr, frame.strideCr,
              m_rgbaBuffer, w, h);
...
// Retry an upload from the already-converted m_rgbaBuffer (no re-conversion).
...
void VideoPlayer::update(float dt) {
    ...
    if (timeToProcess && !m_pendingUpload) {
        // Drain past-due frames, keeping only the most recent one.
        while (true) {
...
void VideoPlayer::play() {
    ...
    // Init the GPU pipeline asynchronously to avoid a first-frame freeze.
    // uploadFrame() falls back to lazy init if the first frame arrives first.
...
void VideoPlayer::resume() {
    ...
    // If finished, seek back to start for reuse.
    if (m_decoder->isFinished() || m_decoder->peekNextPTS() >= DBL_MAX) {
        m_decoder->seekTo(0.0);
        m_playbackTime = 0.0;
        // Clear stale visible-frame flag and PBO fences.
    ...
    // Restart decoder if it isn't running.
    if (!m_playing) {
        m_playing = true;
        m_decoder->startDecoding();
    }
...
void VideoPlayer::releaseGPUResolveCache() {
    // No-op for RGBA / CPU paths.
    if (!m_useGPUYuv) return;

    // GL calls are only valid on the main thread.
    if (!isOnMainThread()) {
    ...
    // m_resolvedRGBA was non-owning; drop it now that the owner is gone.
    m_resolvedRGBA = nullptr;
    m_resolvedAtFrame = 0;
    // m_blitShader is a process-wide singleton — do not release it.
    m_locCb = -1;
    m_locCr = -1;
    m_locY  = -1;
    m_locCS = -1;
