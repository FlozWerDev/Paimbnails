...
struct RoundState {
    ...
    // Absolute deadline (epoch ms in the local client clock, after applying
    // serverClockOffsetMs). 0 means "no active timer". The UI uses this to
    // tick down every frame between snapshots without waiting for the
    // server to push timeLeftSeconds.
...
inline cocos2d::ccColor3B accentColor() {
    // Oro de Geometry Dash (mismo color base que `goldFont.fnt`).
    // Reemplaza el morado PaimonDraw `{139, 92, 246}` para que el mod
    // se sienta nativo a GD.
    return {255, 217, 119};
}
