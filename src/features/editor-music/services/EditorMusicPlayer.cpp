...
// Local alias to avoid clashing with geode::PlaybackMode pulled in by the prelude.
...
bool EditorMusicPlayer::playTrackById(const std::string& trackId) {
    ...
    // Release the previous sound before creating the new one.
    ...
    m_currentDisplayName = track->displayName.empty()
        ...
        : track->displayName;
    m_paused = false;
    m_channel->setPaused(m_mainActive);  // start paused if the main channel is playing
...
bool EditorMusicPlayer::playPrevious() {
    ...
    m_history.pop_back();          // drop the current one
    auto prevId = m_history.back();
    m_history.pop_back();          // playTrackById re-adds it
