...
namespace {
    ...
    // Convierte una string a lowercase ASCII (sin tocar UTF-8 multibyte).
    // Es suficiente para fuzzy match casual sobre nombres de canciones.
...
bool SongSearchRowWidget::init(SongSearchPopup* parent) {
    ...
    // Fondo de la fila — panel oscuro semi-transparente.
    if (auto* bg = paimon::SpriteHelper::createDarkPanel(320.f, 36.f, 130, 5.f)) {
        bg->setPosition({0.f, 0.f});
        this->addChild(bg, -1);
    }

    // Nombre de la canción (línea superior, font grande)
    ...
    // Artista (línea inferior, fuente dorada y más pequeña)
    ...
    // Botón seleccionar (check verde) — atajo accesible junto al play
...
void SongSearchRowWidget::setSong(SongInfoObject* song) {
    ...
    // Limitar el ancho del label de nombre (truncate visual con scale).
...
void SongSearchRowWidget::updatePlayButton() {
    ...
    // Determina si esta canción es la que está sonando como preview desde
    // este popup. Verificamos contra el tracking del padre + el estado real
    // de FMOD (canal 0). Si la música del canal 0 dejó de sonar (por
    // cualquier razón), volvemos al icono "play".
...
void SongSearchRowWidget::ccTouchEnded(CCTouch* touch, CCEvent*) {
    // Tap en cualquier zona del widget (excepto los botones, que el menú
    // interceptó antes) => seleccionar la canción.
...
bool SongSearchPopup::init(SelectCallback callback) {
    // 380x300: cabe sin tapar el ProfileMusicPopup (400x260) — el usuario
    // sigue viendo brevemente el popup padre detrás.
    ...
    // Label con conteo de resultados (esquina superior derecha)
    ...
    // Container donde van los rows (su Y se mueve para hacer scroll)
    ...
    // Mensaje "no hay canciones / sin resultados"
    ...
    // Hint de ayuda al pie del popup
    ...
    // Cargar todas las canciones descargadas
    rebuildScrollList();

    paimon::markDynamicPopup(this);

    // Enable touch para scroll en el área del popup
    this->setTouchEnabled(true);
...
void SongSearchPopup::rebuildScrollList() {
    ...
    // CCArrayExt simplifica la iteración tipada
...
void SongSearchPopup::runSearch() {
    ...
    if (query.empty()) {
        // Sin query: orden alfabético por nombre
        ...
        // Con query: fuzzy match + sort por score
    ...
    // Reset scroll y label de resultados
    m_yScroll = 0.f;
...
void SongSearchPopup::updateScrollLayout(bool forceRefresh) {
    ...
    // Crear el pool de widgets de forma lazy (sólo la primera vez)
    if (m_rowPool.empty()) {
    ...
    // Constante: cada fila ocupa rowHeight + spacing en el eje Y
    const float pitch = kRowHeight + kRowSpacing;

    // Total filas y altura virtual del listado
    int totalRows = static_cast<int>(m_filtered.size());

    // Limitar scroll al rango válido
    ...
    for (int i = 0; i < static_cast<int>(m_rowPool.size()); ++i) {
        ...
        row->setVisible(true);
        // Forzar refresh si el set ha cambiado o cambió de fila
        bool needsUpdate = forceRefresh ||
                           (row->getSong() != m_filtered[rowIndex]);
        if (needsUpdate) {
            row->setSong(m_filtered[rowIndex]);
        }
        // Posición Y dentro del scroll: la primera fila visible queda
        // arriba, las siguientes hacia abajo. El offset fraccional viene
        // del scroll continuo.
...
bool SongSearchPopup::fuzzyMatch(std::string const& query, std::string const& target, int& outScore) {
    ...
    // 1) Substring match exacto: el mejor score posible.
    auto pos = target.find(query);
    if (pos != std::string::npos) {
        outScore = 10000;
        if (pos == 0) {
            outScore += 500;  // bonus por match al inicio
        } else {
            // Penalty leve cuanto más lejos del inicio
            outScore -= static_cast<int>(pos);
        }
        return true;
    }

    // 2) Match de caracteres en orden (todos los chars del query deben
    //    aparecer en target, en el mismo orden, no necesariamente seguidos).
    size_t qIdx = 0;
    int contiguous = 0;
    int matchedAtStart = 0;
    for (size_t tIdx = 0; tIdx < target.size() && qIdx < query.size(); ++tIdx) {
        if (target[tIdx] == query[qIdx]) {
            // Score: cada char vale 5 base + bonus contiguous
...
void SongSearchPopup::textChanged(CCTextInputNode*) {
    // Re-correr búsqueda al cambiar el texto
    runSearch();
}

void SongSearchPopup::onClose(cocos2d::CCObject* sender) {
    // Detener cualquier preview disparado desde aquí. No queremos dejar
    // audio colgado al cerrar el popup.
    auto* fmod = FMODAudioEngine::sharedEngine();
...
bool SongSearchPopup::ccTouchBegan(CCTouch* touch, CCEvent* event) {
    // Geode::Popup maneja el cierre y el sistema de prioridad de touch.
    // Devolvemos true si está dentro del popup para poder hacer scroll.
...
void SongSearchPopup::ccTouchMoved(CCTouch* touch, CCEvent* event) {
    ...
    // Permitir drag-scroll si el touch comenzó dentro del clip.
...
void SongSearchPopup::scrollWheel(float vertical, float /*horizontal*/) {
    // Rueda del mouse: scroll vertical (~3 filas por tick)
...
void SongSearchPopup::onSongPreview(SongInfoObject* song) {
    ...
    // Si esta misma canción ya está sonando, la detenemos (toggle).
    bool sameSongPlaying = (m_currentPreviewSongID == song->m_songID) &&
                           fmod->isMusicPlaying(0);

    // Detener cualquier música que esté en el canal 0 antes de cambiar.
