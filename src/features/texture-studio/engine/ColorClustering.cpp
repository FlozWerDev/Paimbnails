...
ClusterSet ColorClustering::compute(ImageBuffer const& sprite, ClusteringOptions options) {
    ...
    for (int iter = 0; iter < options.maxIterations; ++iter) {
        ...
        // For circular hue averaging: sum sin/cos and atan2 the mean.
        std::vector<float> sumHueX(k, 0.0f), sumHueY(k, 0.0f);

        for (std::size_t i = 0; i < points.size(); ++i) {
            int c = assignment[i];
            counts[c]++;
            float radH = points[i].h * (3.14159265358979323846f / 180.0f);
            sumHueX[c] += std::cos(radH);
            sumHueY[c] += std::sin(radH);
            sumS[c] += points[i].s;
            sumV[c] += points[i].v;
        }
