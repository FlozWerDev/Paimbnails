...
/**
 * ProfileMusicPopup - UI to configure profile music.
 */
...
protected:
    ...
    // Playback time cursor (vertical glowing line) that moves during preview.
    cocos2d::CCNode* m_playbackCursor = nullptr;
    bool m_isPreviewPlaying = false;
    bool m_cursorScheduled  = false;
    // Sine pulse so the cursor "breathes" during playback.
    ...
    // Moving playback cursor (song-preview style)
    ...
    // Format "Artist - Song · 2:04"; omit the suffix if duration is unknown.
