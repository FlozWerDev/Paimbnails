...
void DecoderNDK::stopDecoding() {
    ...
    if (m_thread.joinable()) {
        // timedJoin gives the decode loop up to 3s to exit its 10ms dequeue
        // timeouts and check m_decoding.  If the codec is stuck (driver bug),
        // we detach and let the OS reclaim on process exit rather than block
...
void DecoderNDK::closeInternal() {
    ...
    // If the decode thread was detached, it might still be inside
    // AMediaCodec_* calls.  Calling delete on the codec while a thread is
    // in-flight is a UAF — leak instead and let the OS clean up on exit.
    if (m_decodeThreadDetached.load(std::memory_order_acquire)) {
