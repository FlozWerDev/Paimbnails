...
bool MenuMusicPopup::init(float width, float height) {
    ...
    // Fondo oscuro que cubria todo el m_mainLayer (m_fullBg) ha sido
    // retirado: el usuario no queria el velo negro semitransparente del
    // tamaño del popup. El frame vanilla de GD (m_bgSprite) ya tapa el
    // exterior y el content clipper cubre casi toda el area interior con
    // la imagen borrosa.
...
MenuMusicPopup::DetectedSong MenuMusicPopup::detectActiveSong() const {
    ...
    // 2) Menu loop externo (sawblade-custom-songs-folder, ventilla u otro).
    auto& loop = paimon::menuloop::MenuLoopManager::get();
    std::string current = loop.getCurrentSong();
    out.audioPath = current;

    if (current.empty() || current == "menuLoop.mp3" || loop.isOriginalMenuLoop()) {
        // Vanilla: no hay portada, solo un nombre generico.
    ...
    // El menu-loop puede tener un nombre ya resuelto.
    ...
    // Intentar parsear como ID numerico: las canciones de GD se guardan
    // como "12345.mp3" en la carpeta Songs; MusicDownloadManager
    // resuelve el SongInfoObject con metadata del backend.
    ...
    // IDs con prefijo tipo "DL_1_85111831": extraer la ultima parte numerica.
    if (out.displayName.empty()) {
        ...
        if (lastUnderscore != std::string::npos && lastUnderscore + 1 < stem.size()) {
            ...
            auto idRes = geode::utils::numFromString<int>(tail);
            if (idRes.isOk() && idRes.unwrap() > 0) {
                if (auto* mdm = MusicDownloadManager::sharedState()) {
                    if (auto* info = mdm->getSongInfoObject(idRes.unwrap())) {
                        if (!info->m_songName.empty()) out.displayName = info->m_songName;
                        if (!info->m_artistName.empty()) out.artist = info->m_artistName;
                    }
                }
            }
        }
    ...
    if (out.displayName.empty()) {
        // Ultimo fallback: el nombre que MenuLoopManager tenga seteado
        // (por ejemplo desde override), si no el filename.
        out.displayName = !resolvedName.empty() ? resolvedName : geode::utils::string::pathToString(p.filename());
    }
    if (out.artist.empty()) {
        out.artist = loop.isOverride() ? "Override" : "External track";
    }

    out.hasAnything = !current.empty() && current != "menuLoop.mp3";
    return out;
}
...
void MenuMusicPopup::refreshFromState() {
    ...
    // Cover path (si existe).
    std::string coverPath = detected.coverPath;
    {
        std::error_code coverEc;
        if (!coverPath.empty() && (!std::filesystem::exists(coverPath, coverEc) || coverEc)) {
            coverPath.clear();
        }
    }

    // Fondo interior del popup + hero: misma cover.
    if (m_bg) m_bg->setCoverFromPath(coverPath);
    if (m_hero) {
        m_hero->setCoverFromPath(coverPath);
        const bool isPlaying = detected.hasAnything && !player.isPaused();
        m_hero->setPausedAppearance(!isPlaying);
    }
    if (m_disc) {
        m_disc->setCoverFromPath(coverPath);
    ...
    // Fullscreen backdrop (blur rebasando el popup). Si no hay portada
    // se queda solo con el dim suave, imitando el popup daily/info.
    applyFullscreenCover(coverPath);
...
void MenuMusicPopup::pollExternalSong(float) {
    // Si el MenuMusicPlayer esta en control, su listener ya refresca.
    if (MenuMusicPlayer::get().currentTrack()) return;

    auto& loop = paimon::menuloop::MenuLoopManager::get();
    std::string current = loop.getCurrentSong();
    if (current == m_lastDetectedPath) return;
...
namespace {
    ...
    static std::string resolveCurrentTrackPath(bool* isMenuLoopVanilla = nullptr) {
        ...
        auto& loop = paimon::menuloop::MenuLoopManager::get();
        if (loop.isOriginalMenuLoop() || loop.getCurrentSong() == "menuLoop.mp3") {
...
void MenuMusicPopup::onCopyName(CCObject*) {
    ...
    std::string name;
    if (auto* t = player.currentTrack()) {
        name = t->displayName.empty()
            ? geode::utils::string::pathToString(std::filesystem::path(t->audioPath).stem())
            : t->displayName;
    } else {
        auto& loop = paimon::menuloop::MenuLoopManager::get();
        if (loop.isOriginalMenuLoop()) name = "menuLoop.mp3";
        else {
            auto disp = loop.getCurrentSongDisplayName();
            name = disp.empty()
                ? geode::utils::string::pathToString(std::filesystem::path(loop.getCurrentSong()).stem())
                : disp;
        }
    }
    geode::utils::clipboard::write(name);
    Notification::create(fmt::format("Copied: {}", name), NotificationIcon::Success)->show();
}
