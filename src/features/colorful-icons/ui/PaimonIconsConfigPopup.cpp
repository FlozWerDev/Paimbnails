...
// ─────────────────────────────────────────────────────────────
// Small UI primitives - lightweight wrappers used by every tab. None of
// these are sophisticated; they exist so the tab builders read like a list
// of declarative rows rather than a maze of cocos boilerplate.
// ─────────────────────────────────────────────────────────────
...
CCNode* makeSliderRow(std::string const& label, float minVal, float maxVal, float current,
    ...
    // Slider needs to live in a wrapper because its anchor is weird.
    ...
    if (slider) {
        ...
        // Slider, so we schedule a tiny polling block on the slider's thumb.
        // It's cheap (one slider per row) and avoids subclassing.
...
// ─────────────────────────────────────────────────────────────
// Tab builders. Each returns a fresh CCNode; the popup owns it.
// ─────────────────────────────────────────────────────────────
...
CCNode* PaimonIconsConfigPopup::buildGeneralTab() {
    ...
    // Show controls relevant to the active mode. We only show a couple of
    // representative ones here; rebuilding the body live each time the
    // mode changes is the cleanest UX but adds glue. For now we always
    // show all knobs; they're cheap.
...
CCNode* PaimonIconsConfigPopup::buildPresetsTab() {
    ...
    if (spr) {
        ...
        auto* btn = CCMenuItemExt::createSpriteExtra(spr,
            [](CCMenuItemSpriteExtra*) {
                // Quick "preset N" name. A real popup would prompt for a name
                // but that requires a TextInput popup; we keep MVP simple.
    ...
    // List presets (very basic; no scroll).
