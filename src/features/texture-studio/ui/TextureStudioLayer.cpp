...
bool TextureStudioLayer::init() {
    ...
    if (m_buttonMenu) {
        if (auto* spr = ButtonSprite::create("+ New Pack", "bigFont.fnt", "GJ_button_01.png", 0.5f)) {
            if (auto* btn = CCMenuItemExt::createSpriteExtra(spr,
                ...
                m_buttonMenu->addChildAtPosition(btn, Anchor::TopRight, {-42.f, -18.f});
            }
        }
    }

    // ── Slot grid ──────────────────────────────────────────────────────
    constexpr float kGridW = 340.f;
    constexpr float kGridH = 175.f;
    ...
    if (m_grid) {
        ...
        m_mainLayer->addChildAtPosition(m_grid, Anchor::Center, {0.f, -2.f});
    }

    // ── Footer ─────────────────────────────────────────────────────────
    if (auto* activeLbl = CCLabelBMFont::create("Active: (none)", "bigFont.fnt")) {
        activeLbl->setScale(0.35f);
        activeLbl->setAnchorPoint({0.f, 0.5f});
        m_mainLayer->addChildAtPosition(activeLbl, Anchor::BottomLeft, {12.f, 15.f});
    ...
    if (m_buttonMenu) {
        if (auto* folderSpr = ButtonSprite::create("Folder", "bigFont.fnt", "GJ_button_05.png", 0.5f)) {
            if (auto* folderBtn = CCMenuItemExt::createSpriteExtra(folderSpr,
                ...
                m_buttonMenu->addChildAtPosition(folderBtn, Anchor::BottomRight, {-40.f, 18.f});
            }
        }
    }
...
void TextureStudioLayer::onEditSlot(std::string const& slotId) {
    ...
    // The grid will refresh next time the user reopens TextureStudioLayer
    // (or we could listen for editor close events; not critical for MVP).
    if (m_grid) m_grid->refresh();
}
...
void TextureStudioLayer::refreshFooter() {
    ...
    if (active.empty()) {
        ...
        m_activeLbl->setString(("Active: " + active).c_str());
    }
}
