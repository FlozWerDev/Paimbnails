...
// GD-native theme palette built from untinted GD assets and canonical colors.
namespace theme {
    // Panels: pure white (untinted) + opacity.
...
// Background: MenuGameLayer, falling back to GJ_gradientBG.png.
...
// Framed panel; falls back NineSlice -> CCScale9Sprite -> rounded rect.
...
// Drag-bar section strip with a centered goldFont title.
CCNode* makeSectionStrip(std::string const& title, float width,
    ...
    float stripH = 26.f;
    bool placed = false;
    // Short insets for the drag bar corners.
...
void fillScroll(geode::ScrollLayer* scroll, std::vector<CCNode*> const& rows, float rowHeight, float padding = 6.f) {
    ...
    for (auto* row : rows) {
        // Center-anchor so the row spans [0, width] within the scroll.
...
// Chip: tinted GJ_square05.png capsule, falls back to flat rounded rect.
...
// Circular icon button: rounded GJ_square05.png panel plus icon.
CCNode* makeIconCircleNode(char const* iconFrame, cocos2d::ccColor3B baseColor,
    ...
    // Return null if the icon frame fails to load.
    return nullptr;
}
...
void PaiDrawLobbyLayer::buildLayout() {
    ...
    // Header logo (PaiDraw icon + title)
    ...
    // Layout: central white panel with the player list
    ...
    // Gold counter at the right of the drag bar
    ...
    // Connection status below the header
    ...
    // Bottom buttons: Join Room (GJ_button_01) and Create Room (GJ_button_02)
    ...
    // Refresh button at the right of the header
...
void PaiDrawLobbyLayer::onExit() {
    // Unsubscribe if the layer is destroyed without onBack. unsubscribe is idempotent.
    paimon::EventBus::get().unsubscribe(m_connectionSub);
    paimon::EventBus::get().unsubscribe(m_lobbySub);
    CCLayer::onExit();
}
...
CCNode* PaiDrawLobbyLayer::createPlayerRow(PlayerInfo const& player, float width, float height) {
    ...
    // GD comment-cell row (GJ_commentCell_001.png), falling back to square03/01.
    ...
    // Subtitle: player level with star icon
...
// PaiDrawRoomsLayer: list of active rooms to join
...
void PaiDrawRoomsLayer::onExit() {
    // Unsubscribe if destroyed without onBack.
    paimon::EventBus::get().unsubscribe(m_connectionSub);
    paimon::EventBus::get().unsubscribe(m_lobbySub);
    CCLayer::onExit();
}
...
CCNode* PaiDrawRoomsLayer::createRoomRow(RoomInfo const& room, float width, float height) {
    ...
    // Join/Lock button: grey if password-protected, else green.
    ...
    // Status chip: green = waiting, red = in game
...
void PaiDrawCreateRoomLayer::buildLayout() {
    ...
    // PaiDraw logo left of the title
    ...
    // Layout: central white panel with two columns
    constexpr float kBottomBar = 56.f;
    ...
    // Drag bar reading "ROOM SETTINGS"
    ...
    float innerTop = panelY + panelH - kPad - 28.f; // space for the drag bar
        ...
        cocos2d::ccColor3B titleColor)
    {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            // GJ_button_04 = inactive; swaps to GJ_button_01 when selected.
    ...
    // Final button, separate from the panel
...
void PaiDrawRoomLayer::buildLayout() {
    ...
    // Drag bars for each panel header
    ...
    // Bottom buttons: Ready (green), Start (blue), Settings (grey)
    float btnY = kBottomBar / 2.f - 4.f;
    ...
    // Settings: gear icon button, like GD's main-menu "OPTIONS".
...
CCNode* PaiDrawRoomLayer::createPlayerRow(PlayerInfo const& player, float width, float height) {
    ...
    if (auto* cell = paimon::SpriteHelper::safeCreateNineSlice(
        ...
        // Subtle gold tint marks the host.
    ...
    // Player avatar (SimplePlayer icon)
...
void PaiDrawRoomLayer::rebuildChat() {
    ...
    for (auto const& message : messages) {
        ...
        // Message cell; subtle tint when the guess is correct (green) or close (gold).
        ...
        // Sender in gold or white depending on message type
...
void PaiDrawRoomLayer::onExit() {
    // Unsubscribe if destroyed without onBack.
    // Don't call leaveRoom() here; onBack handles that on the normal path.
    paimon::EventBus::get().unsubscribe(m_roomSub);
    paimon::EventBus::get().unsubscribe(m_chatSub);
    CCLayer::onExit();
}
...
bool PaiDrawCanvasNode::init() {
    ...
    // Canvas background: flat white rounded rect (CCDrawNode keeps it gradient-free).
    ...
    // Decorative beveled GJ_square05.png frame over the paper (NineSlice, falls back to CCScale9Sprite).
    ...
    // Clipper to contain strokes within the canvas area
    ...
    // Live preview layer (lines, shapes)
...
void PaiDrawCanvasNode::clearRedoStack() {
    // Redo-stack groups are detached and only owned here; release directly.
...
void PaiDrawCanvasNode::undoLast() {
    ...
    if (group) {
        // retain() offsets removeFromParentAndCleanup()'s implicit release,
        // leaving refcount 1. Don't release here (dangling pointer in redo/clear).
        group->retain();
...
PaiDrawCanvasNode::~PaiDrawCanvasNode() {
    // Release redo-stack groups detached from the scene.
    for (auto* node : m_redoStack) {
        if (node) node->release();
    }
    m_redoStack.clear();
...
void PaiDrawCanvasNode::redoLast() {
    ...
    // Release the reference the redo stack held (balances undoLast's retain()).
...
void PaiDrawCanvasNode::addStroke(StrokeSegment const& stroke) {
    ...
    // Bake opacity into the color (CCDrawNode ignores setOpacity on drawn primitives).
    ...
    // One capsule per segment: uniform thickness, no alpha buildup from overlapping circles.
...
bool PaiDrawCanvasNode::ccTouchBegan(CCTouch* touch, CCEvent*) {
    ...
    if (m_tool == Tool::Pencil || m_tool == Tool::Eraser) {
        beginStrokeGroup();
        // Short click leaves a visible dot at the start point.
...
bool PaiDrawGameLayer::init() {
    ...
    // Tick the timer label locally every 100ms for smooth countdown between server snapshots.
...
void PaiDrawGameLayer::buildLayout() {
    ...
    // Layout: side rails + central drawing area
    ...
    // Drawing-area frame: untinted GJ_square01.png panel.
    ...
    m_canvasZoom = 1.f;
    // Cache canvas rect so applyCanvasZoom() can recompute the anchor.
    ...
    // 16-color palette from GD's color picker.
    std::vector<cocos2d::ccColor3B> colors = {        {  0,   0,   0}, {255, 255, 255},   // black / white
        {255,  35,  35}, {255, 100,   0},   // GD red / GD orange
        {255, 165,   0}, {255, 235,   0},   // amber / GD yellow
        {120, 230,   0}, {  0, 200,  60},   // GD lime / GD green
        {  0, 200, 220}, {  0, 165, 255},   // GD cyan / GD blue
        { 50, 100, 255}, {120,  60, 220},   // deep blue / GD indigo
        {175,   0, 255}, {255,   0, 175},   // GD violet / GD pink
        {255, 105, 165}, {165, 100,  60}    // pastel pink / GD brown
    };
    ...
    for (auto const& color : colors) {
        // GJ_colorBtn_001.png tinted to the swatch color.
    ...
    // 3 brushes below the palette; black dot grows with the size.
    ...
    for (float dotR : {3.f, 5.5f, 9.f}) {
        ...
        // White GJ_square05.png mini-background
        ...
        // Brush-size dot on white background.
    ...
    // Tool buttons: eraser and clear canvas.
    ...
    {
        // Try iconFrame first, fall back to fallbackFrame, then a text button.
        ...
        // Wrap icon in a circle panel; no tint so native colors are preserved.
        auto* node = makeIconCircleNode(chosenFrame, theme::kPanelTint, 32.f, 0.78f);
        if (!node) {
            // Fallback (shouldn't happen since we verified chosenFrame exists).
            auto* btnSpr = ButtonSprite::create("?", "bigFont.fnt", "GJ_button_04.png", 0.7f);
            btnSpr->setScale(0.55f);
            auto* btn = CCMenuItemSpriteExtra::create(btnSpr, this, cb);
            btn->setTag(tag);
...
void PaiDrawGameLayer::applyCanvasZoom() {
    if (!m_canvas) return;
    // Recompute anchor from the cached canvas rect; fall back to m_canvasAnchor if uninitialized.
...
void PaiDrawGameLayer::refreshHeader() {
    ...
    if (m_timerLabel) {
        // Prefer local absolute deadline for smooth updates between snapshots.
    ...
    // Allow drawing regardless (test mode).
    if (m_canvas) {
...
void PaiDrawGameLayer::rebuildScoreboard() {
    // Removed to give the canvas more space.
}
...
void PaiDrawGameLayer::onColor(CCObject* sender) {
    // Same palette as buildLayout.
    static std::vector<cocos2d::ccColor3B> colors = {        {  0,   0,   0}, {255, 255, 255},
        {255,  35,  35}, {255, 100,   0},
        {255, 165,   0}, {255, 235,   0},
        {120, 230,   0}, {  0, 200,  60},
        {  0, 200, 220}, {  0, 165, 255},
...
void PaiDrawResultsLayer::buildLayout() {
    ...
    // Layout: podium on top + table below
    ...
    // Stats bar between podium and table
...
void PaiDrawResultsLayer::rebuildTable() {
    ...
    for (size_t index = 0; index < results.leaderboard.size(); ++index) {
        ...
        // GD comment cell; top 3 get a subtle gold tint
        bool placedRowBg = false;
        ...
        // Rank label, gold/silver/bronze for top 3.
        ...
        else if (index == 1) rankColor = {220, 220, 220}; // silver
        else if (index == 2) rankColor = {200, 130, 80};  // bronze
